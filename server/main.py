...
# --- CONFIGURACIÓN ---
MIN_JUGADORES = 7  
TIEMPO_VOTO = 900  # 15 minutos en segundos
# ---------------------
...
async def unirse_juego(jugador: Jugador):
    ...
    return {"status": "ok", "jugadores": lista_jugadores, "fase": fase, "ya_listo": nombre in jugadores_listos}
...
async def iniciar_votacion(data: dict):
    ...
    if tarea_temporizador:
        tarea_temporizador.cancel()
...
async def votar(data: dict):
    ...
    if not frase_actual["nombre"]: return {"status": "no_active_vote"}
    ...
    # Si votan todos (menos el que reclama), cerramos manual y cancelamos reloj
